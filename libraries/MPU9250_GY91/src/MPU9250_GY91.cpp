...
#define MPU9250_DEFAULT_WDELAY    10
#define MPU9250_PI_VALUE          3.14159


uint8_t MPU9250_GY91::i2cRead(uint8_t Address, uint8_t Register, uint8_t Nbytes, uint8_t* Data) 
{
...
uint8_t MPU9250_GY91::i2cWriteByte(uint8_t Address, uint8_t Register, uint8_t Data) 
{
  myWire->beginTransmission(Address);
  myWire->write(Register);
  myWire->write(Data);
...
MPU9250_GY91::MPU9250_GY91(uint8_t address):address(address) 
{
  accelRange = 0;
  gyroRange  = 0;
  magXOffset = 0;
...
void MPU9250_GY91::setWire(TwoWire* wire) 
{
  myWire = wire;
}

uint8_t MPU9250_GY91::readId(uint8_t *id) 
{
  beginWireIfNull();
  return i2cRead(address, MPU9250_ADDR_WHOAMI, 1, id);
}

void MPU9250_GY91::beginWireIfNull() 
{
  if (myWire == NULL) 
  {
    myWire = &Wire;
...
void MPU9250_GY91::beginAccel(uint8_t mode) 
{
  beginWireIfNull();
  switch(mode) {
  case ACC_FULL_SCALE_2_G:
    accelRange = MPU9250_SCALE_V2;
    break;
  case ACC_FULL_SCALE_4_G:
    accelRange = MPU9250_SCALE_V4;
    break;
  case ACC_FULL_SCALE_8_G:
    accelRange = MPU9250_SCALE_V8;
    break;
  case ACC_FULL_SCALE_16_G:
    accelRange = MPU9250_SCALE_V16;
    break;
  default:
    return; // Return without writing invalid mode
  }
  i2cWriteByte(address, MPU9250_ADDR_ACCELCONFIG, mode);
  delay(MPU9250_DEFAULT_WDELAY);
}

void MPU9250_GY91::magReadAdjustValues() 
{
...
void MPU9250_GY91::beginMag(uint8_t mode) 
{
  beginWireIfNull();
  magWakeup();
  magEnableSlaveMode();
...
void MPU9250_GY91::magSetMode(uint8_t mode) 
{
  i2cWriteByte(AK8963_ADDRESS, AK8963_RA_CNTL1, mode);
  delay(MPU9250_DEFAULT_WDELAY);
}

void MPU9250_GY91::magWakeup() 
{
  unsigned char bits;
...
void MPU9250_GY91::magEnableSlaveMode() 
{
  unsigned char bits;
...
float MPU9250_GY91::magHorizDirection() 
{
  const float Pi = MPU9250_PI_VALUE;
  return atan2(magX(), magY()) * 180 / Pi;
}

uint8_t MPU9250_GY91::magUpdate() 
{
  return i2cRead(AK8963_ADDRESS, AK8963_RA_HXL, 7, magBuff);
}

int16_t MPU9250_GY91::magGet(uint8_t highIndex, uint8_t lowIndex) 
{
...
uint8_t MPU9250_GY91::gyroUpdate() 
{
  return i2cRead(address, MPU9250_ADDR_GYRO_XOUT_H, 6, gyroBuff);
}

float MPU9250_GY91::gyroGet(uint8_t highIndex, uint8_t lowIndex) 
{
  int16_t v = ((int16_t) gyroBuff[highIndex]) << 8 | gyroBuff[lowIndex];
  return ((float) -v) * gyroRange / (float) 0x8000;
}

float MPU9250_GY91::gyroX() 
{
  return gyroGet(0, 1);
}

float MPU9250_GY91::gyroY() 
{
  return gyroGet(2, 3);
}

float MPU9250_GY91::gyroZ() 
{
  return gyroGet(4, 5);
}
